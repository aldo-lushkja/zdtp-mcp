...
public class UserStoryMcpTools {
    ...
    public void register(McpServer server, SchemaBuilder schema) {
        server.registerTool("user_story_search", "Search for user stories in Targetprocess.",
                schema.object().prop("nameQuery", schema.string()).prop("projectName", schema.string()).prop("ownerLogin", schema.string())
                        ...
                        .prop("sprintId", schema.integer()).prop("featureId", schema.integer()).prop("take", schema.integer().withDefault(10)).build(),
        ...
        server.registerTool("user_story_create", "Create a new user story.",
                schema.object().prop("name", schema.string().required()).prop("projectId", schema.integer().required())
                        ...
                        .prop("featureId", schema.integer()).prop("teamIterationId", schema.integer()).prop("teamId", schema.integer())
                        .prop("releaseId", schema.integer()).build(),
    ...
    private String create(String n, int p, String d, Double e, Integer fi, Integer ti, Integer team, Integer ri) { return "Created: " + format(createService.create(n, p, d, e, fi, ti, team, ri)); }
    private String update(int i, String n, String d, String s, Double e, Integer fi, Integer ti, Integer team, Integer ri) { return "Updated: " + format(updateService.update(i, n, d, s, e, fi, ti, team, ri)); }
