...
public class UserStorySearchService extends BaseService {
    ...
    public UserStorySearchService(TargetProcessProperties props, TargetProcessHttpClient http, UserStoryConverter conv, ObjectMapper mapper) {
    ...
    public List<UserStoryDto> search(SearchCriteria criteria) {
        String where = query().add("Name", "contains", criteria.nameQuery()).add("Project.Name", "contains", criteria.projectName()).add("Owner.Login", "eq", criteria.ownerLogin())
                ...
                .add(criteria.sprintId() != null && criteria.sprintId() > 0 ? "TeamIteration.Id eq %d".formatted(criteria.sprintId()) : null).build();
        ...
        return engine.list(QueryEngine.USER_STORY, p, new TypeReference<>() {}, converter::toDto);
    }
}
